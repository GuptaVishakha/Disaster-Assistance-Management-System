...
@bp.route("/createPledge", methods=["GET","POST"])
def createPledge():
...
def modifypledge():
    cursor.execute("SELECT * FROM pledge WHERE username = '%s'" % username)
    namematch = cursor.fetchall()
    error = None
    if len(namematch) == 0:
        error = "You have not made a pledge yet"
        flash(error)
        return redirect(url_for("home.home"))

    if error is None:
        return render_template("modifypledge.html")

@bp.route("/change",methods=["POST","GET"])
def changeItems():
    cursor.execute("SELECT * FROM pledge WHERE username = 'test'")
    ...
    cursor.execute("SELECT items FROM pledge WHERE username = '%s'" % username)
    ...
    if request.method == "POST":
        ...
        if error is None:
            cursor.execute("DELETE FROM pledge WHERE username = '%s'" % username)
            mydb.commit()
            cursor.execute("INSERT INTO pledge (username, event, items, description)  VALUES (%s,%s,%s,%s)", (username,eventname,str(items).strip("[]"),description))
            mydb.commit()
            msg = "pledge Updated Succesfully"
            flash(msg)
            return redirect(url_for("home.home"))
        flash(error)
    return render_template("changeitems.html", itemlist=newitems)
@bp.route("/deletepledge",methods=["POST","GET"])
def delete():
    cursor.execute("SELECT * FROM pledge WHERE username = '%s'" % username)
    ...
    if request.method == "POST":
        if request.form["submit"] == "Yes":
            ...
            return redirect(url_for("home.home"))
    return render_template("deletepledge.html", requests=newrequestformat)
...
def pledgehelp():
    ...
    if request.method == "POST":
        ...
        if description == "THIS IS A DESCRIPTION FOR THE pledge CURRENTLY BEING MADE":
            global username
            username = "test"
        cursor.execute("SELECT * FROM pledge WHERE username = '%s'" % username)
        possname = cursor.fetchall()
        if len(possname) != 0:
            error = "User has already made a pledge"
